...
export default function AdminPanel() {
  ...
  const reject = async (id) => {
    ...
    await adminApi.rejectProvider(id, reason);
    fetchData("providers");
  };
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-semibold text-gray-900">Provider #{p.id}</span>
                            ...
                            <div className="text-sm text-gray-600 space-y-0.5 mt-1">
                              ...
                              {p.bio && <p className="text-gray-400 line-clamp-2 mt-1">{p.bio}</p>}
                              {p.rejection_reason && (
                                <p className="text-red-500 text-xs">Rejection reason: {p.rejection_reason}</p>
                              )}
                            </div>
                          </div>
                          <div className="flex flex-col gap-2 flex-shrink-0">
                            {p.status === "pending" && (
                              <>
                                <button className="btn-primary text-xs py-1.5 px-3 bg-green-600 hover:bg-green-700" onClick={() => approve(p.id)}>
                                  Approve
                                </button>
                                <button className="btn-ghost text-xs py-1.5 px-3 text-red-500" onClick={() => reject(p.id)}>
                                  Reject
                                </button>
                              </>
                            )}
                            {p.status === "approved" && (
                              <button
                                ...
                                onClick={() => boost(p.id, !p.is_boosted)}
                              >
